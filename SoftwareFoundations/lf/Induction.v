...
Proof.
  intros; 
  induction n; [auto | simpl; rewrite <- plus_n_Sm; auto].
Qed.
...
Theorem add_assoc1 : forall n m p : nat,
  n + (m + p) = (n + m) + p.
Proof.
...
Proof.
  intros n m p.
  rewrite add_assoc.
  assert(H: n + m = m + n) by (rewrite add_comm; reflexivity).
  rewrite H; clear H.
  rewrite add_assoc.
  reflexivity.
Qed.
